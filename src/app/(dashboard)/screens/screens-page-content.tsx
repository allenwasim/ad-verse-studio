...
export function ScreensPageContent({ allScreens, campaigns, leads, onScreenSaved }: { allScreens: Screen[], campaigns: Campaign[], leads: Lead[], onScreenSaved: () => void }) {
  const searchParams = useSearchParams();
  const search = searchParams.get('search') || undefined;
  
  const [sheetOpen, setSheetOpen] = useState(false);
  const [selectedScreen, setSelectedScreen] = useState<Screen | undefined>(undefined);

  const screens = allScreens.filter(screen => {
    if (!search) return true;
    const searchTerm = search.toLowerCase();
    return screen.venueName.toLowerCase().includes(searchTerm) || screen.location.toLowerCase().includes(searchTerm);
  });
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Manage your advertising screens and venues. Click a row to see scheduled ads.
              </CardDescription>
            </div>
          ...
          <Accordion type="single" collapsible className="w-full"> 
            {screens.map((screen) => {
              const activeCampaigns = campaigns.filter(
                (campaign) =>
                  campaign.assignedScreens.includes(screen.id) && new Date(campaign.endDate) >= new Date() && new Date(campaign.startDate) <= new Date()
              );
              const allAssignedCampaigns = campaigns.filter(c => c.assignedScreens.includes(screen.id) && new Date(c.endDate) >= new Date());
                ...
                <AccordionItem value={screen.id} key={screen.id}>
                    <div className="flex items-center gap-4 px-4 py-2 hover:bg-muted/50 data-[state=open]:bg-muted border-b">
                        ...
                        <div className="flex-1 font-medium truncate">
                            ...
                            <div className="text-sm text-muted-foreground truncate">{screen.location}</div>
                        </div>

                        <div className="hidden sm:flex items-center justify-end gap-2">
                            {screen.venueType && (
                                <Badge variant="outline" className="capitalize">{screen.venueType}</Badge>
                            )}
                        ...
                        <div className="w-10">
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                    <EditScreenMenuItem onSelect={() => handleEditClick(screen)} />
                                    <DeleteDialogWrapper
                                    onConfirm={async () => {
                      ...
                      <div className="p-4 bg-muted/50 space-y-4">
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
                              <div>
                                  ...
                                  <div>{screen.averageFootfall?.toLocaleString() || 'N/A'}</div>
                              </div>
                              <div>
                          ...
                          <div>
                              <h4 className="font-bold text-sm mb-2">Scheduled Ads</h4>
                              <ScreenAdsList campaigns={allAssignedCampaigns} leads={leads} />
                          </div>
                      </div>
                  </AccordionContent>
                </AccordionItem>
