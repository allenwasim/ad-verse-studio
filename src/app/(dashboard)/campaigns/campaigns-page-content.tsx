...
export function CampaignsPageContent({
  ...
  const getClientName = (clientId: string) => {
    // Try campaign.clientName first, then leadName, then 'Unknown'
    const campaign = allCampaigns.find((c) => c.clientId === clientId);
  ...
  const handleMediaInteraction = useCallback(
    (url: string | undefined, campaignName: string) => {
      if (!url) {
            ...
            <TableBody>
              {campaigns.map((campaign) => (
                <React.Fragment key={campaign.id}>
                  <TableRow
                    className={expandedRows[campaign.id] ? "border-b-0" : ""}
                  >
                    <TableCell>
                      <Button
                        variant="ghost"
                      ...
                      <TableCell colSpan={6}>
                        <div className="p-4 grid grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <div>
                              <ClientDate date={campaign.startDate} />
                            </div>
                          </div>
                          <div>
                            <div className="font-bold">End Date</div>
                            <div>
                              <ClientDate date={campaign.endDate} />
                            </div>
                          </div>
