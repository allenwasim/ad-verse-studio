...
export function CampaignSheet({
  ...
  async function handleAction(prevState: any, formData: FormData) {
    ...
    selectedScreens.forEach((screen) =>
      newFormData.append("assignedScreens", screen)
    );

    // Validate media upload before saving
    if (!mediaURL || !mediaStoragePath) {
      toast({
        variant: "destructive",
        title: "Media Required",
        description: "Please upload a valid media file before saving.",
      });
      return { message: "error" };
    }
    ...
    startTransition(async () => {
      ...
      if (!campaignId || selectedScreens.length === 0) {
        toast({
          ...
          description:
            "Please save the campaign and select at least one screen to check compatibility.",
        });
        return;
      }

      const result = await checkAdScreenCompatibility(
        campaignId,
        selectedScreens
      );
      ...
      if ("error" in result) {
        ...
        if (result.isValid) {
          toast({
            ...
            description: "All selected screens are valid for this campaign.",
          });
        } else {
          const invalidScreenNames = result.invalidScreenIds
            .map(
              (id: string) => screens.find((s) => s.id === id)?.venueName || id
            )
            .join(", ");
          toast({
            variant: "destructive",
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="sm:max-w-lg flex flex-col">
        <SheetHeader>
          ...
          <SheetDescription>
            {campaign
              ? "Update the details of the existing campaign."
              : "Create a new ad campaign and contract."}
          </SheetDescription>
        </SheetHeader>
          ...
          <form ref={formRef} action={formAction} className="space-y-4 py-4">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <Select
                  name="paymentStatus"
                  defaultValue={campaign?.paymentStatus}
                >
                  <SelectTrigger>
            ...
            <div className="flex items-center space-x-2 pt-2">
              <Checkbox
                ...
                defaultChecked={campaign?.renewalReminder}
              />
              <label
